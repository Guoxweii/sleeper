...
- 当前前端提交的 `datetime-local` 值不带时区，服务端解析时依赖部署机器本地时区，可能导致写入的 UTC 时间偏移。
- 影响：会直接影响睡眠记录时间、重叠校验，以及周/月分析归属结果。

2. 周/月分析中的“平均每日睡眠”口径偏乐观

- 当前实现按“有记录的天数”计算平均值，而页面和文案表达更接近“周期内每天平均”。
- 影响：当漏记较多天时，分析结果会偏乐观，评分和建议也会受到影响。

3. 前端周/月切换和分页加载存在请求竞态

- 在快速切换 Board、周/月、筛选条件或分页时，较早发出的请求可能晚于新请求返回，并覆盖新状态。
- 影响：页面可能展示错误的列表或错误周期的分析结果。

4. 请求失败后页面仍保留旧数据

- 当前页面在接口报错时会显示错误消息，但通常不会清空上一轮成功请求留下的数据。
- 影响：用户可能误把旧数据当成当前筛选条件下的有效结果。

5. AI 查询内容的统计周期字段与后端返回结构未完全对齐

- 当前 AI 查询文案生成逻辑依赖的统计周期字段，与周/月分析接口返回的数据结构不完全一致。
- 影响：复制出来的 AI 查询内容可能缺少准确的统计周期信息，影响外部分析上下文。
