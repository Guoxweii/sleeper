...
import { boardResponseSchema, weeklyAnalysisResponseSchema } from '../../../shared/index.ts'
import type { AnalysisReview, AnalysisSourceRecord, AnalysisTypeTotals, Board, ReviewStatus, WeeklyAnalysis } from '../../../shared/index.ts'
...
const analysis = ref<WeeklyAnalysis | null>(null)
...
async function fetchAnalysisData(targetBoardId: number, targetWeek: string) {
  return api.get(
...
function isCurrentLoad(loadId: number, targetBoardId: number, targetWeek: string): boolean {
  return loadId === latestAnalysisLoadId && targetBoardId === boardId.value && targetWeek === week.value
}
...
async function loadPage(): Promise<void> {
  ...
  try {
    const [boardResponse, analysisResponse] = await Promise.all([
      ...
      fetchAnalysisData(targetBoardId, targetWeek)
    ])
    ...
    if (isCurrentLoad(analysisLoadId, targetBoardId, targetWeek)) {
      analysis.value = analysisResponse.analysis
    }
  } catch (error) {
    if (
      pageLoadId !== latestPageLoadId ||
      !isCurrentLoad(analysisLoadId, targetBoardId, targetWeek) ||
...
async function reloadAnalysisOnly(): Promise<void> {
  ...
  try {
    const response = await fetchAnalysisData(targetBoardId, targetWeek)
    if (!isCurrentLoad(loadId, targetBoardId, targetWeek)) {
      return
    }
